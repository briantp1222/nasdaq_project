...
if __name__ == '__main__':
    ...
    for key in all_tables:
        ...
        try:
            tasks.append(update_data_2.to_deployment(name=f"updating {key.replace('/', '_').lower()}", cron=cron,
                                                   parameters={'key': key}))
        except:
            tasks.append(update_data_2.to_deployment(name=f"updating {key.replace('/', '_').lower()}", cron='00 00 * * *',
                                                   parameters={'key': key}))
    serve(*tasks)